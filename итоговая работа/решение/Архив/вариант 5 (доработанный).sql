...
select "день", z2.departure_airport, z2.num, z2.summ
from
	(select f.actual_departure::date as "день",
	f.departure_airport, s1.num, s1.aircraft_code,
	count(f.actual_departure::date) over (partition by f.departure_airport order by f.actual_departure::date) as count_date,
	sum(num) over (partition by f.departure_airport order by f.actual_departure::date) as summ
	from flights f
	...
	GROUP by "день", f.departure_airport, s1.num, s1.aircraft_code
	order by "день") z2
where z2.summ != z2.num
