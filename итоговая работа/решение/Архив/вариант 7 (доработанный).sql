...
--старый вариант
select f1.день, f1.departure_airport,
sum(sum(f1.total_seats)) over (partition by f1.departure_airport order by f1.день),
sum(f1.total_seats)
from
	(select f.actual_departure::date as "день", f.departure_airport, s1.total_seats, t_b.total_bookings
	from flights f
	join (
	...
	and t_b.total_bookings is null) f1
GROUP by f1.день, f1.departure_airport, f1.total_seats
...
from 
	(select *,
		case --не соответствует условию задачи
			...
			when sum between 50000000 and 149999999 then 'middle'
			else 'high'
		end
	from cte1) x1
GROUP by x1.case
