...
select b1.actual_departure::date, b1.departure_airport, b1.total_seats,
sum(b1.total_seats) over (partition by b1.actual_departure::date, b1.departure_airport order by b1.actual_departure)
from 
	(select f.actual_departure, f.departure_airport, s1.total_seats
	,count(f.departure_airport) over (partition by f.actual_departure::date, f.departure_airport) as count_da
	from flights f 
	left join (select bp.flight_id, count(bp.boarding_no) as b_no
				from boarding_passes bp
				group by bp.flight_id)	bp1 on f.flight_id = bp1.flight_id
	left join(select s.aircraft_code, count(*) as total_seats
				from seats s 
				group by s.aircraft_code) s1 on f.aircraft_code = s1.aircraft_code
	where f.actual_departure is not null
	and bp1.b_no is null
	order by f.actual_departure::date) b1
where b1.count_da > 1
...
from 
	(select *,
		case --не соответствует условию задачи
			when sum < 50000000.00 then 'low'
			when sum between 50000000.00 and 149999999.99 then 'middle'
			when sum >= 150000000.00 then 'high'
		end
	from cte1) x1
GROUP by x1.case
